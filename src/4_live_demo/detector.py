...
class VehicleDetector:
    ...
    def _calculate_logic(self, box, c_name, w_img, h_img, model_dist=0.0):
        # 1. 공통 변수 추출 (어떤 로직을 쓰든 좌표는 필요하니까 미리 뽑음)
        x1, y1, x2, y2 = box
        box_w = x2 - x1
        box_h = y2 - y1
        ...
        # 2. 결과 변수 초기화 (에러 방지용)
        final_dist = 0.0
        dist_geo = 0.0 # Entering 판단에 필요해서 미리 선언
        
        # [스위치] True: 기존 수학 로직 / False: 미래의 새로운 로직
        USE_MATH_LOGIC = True 
        ...
        if USE_MATH_LOGIC:
            ...
            # [B] 미래에 다른 담당자가 채워넣을 공간
            # 여기서 final_dist 와 dist_geo를 계산해줘야 함
            pass
            # 예: final_dist = new_algorithm(box, ...)
            # 예: dist_geo = ... (Entering 로직을 위해 필요하다면)
        ...
        if is_touching_side:
            ...
            if is_really_close:
                # 주의: 여기서 dist_geo를 사용하므로, 
                # 위쪽 else 블록에서도 dist_geo 값은 계산되어 있어야 함!
